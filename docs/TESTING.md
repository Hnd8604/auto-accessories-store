...
Toàn bộ 21 service còn lại đã có test. Toàn backend hiện có **261 test, 0 failure** (2 test skip: `StoreApplicationTests` bị `@Disabled`, `SchemaMigrationTest` chỉ chạy khi có `TEST_DB_URL`).
...
| | `ProfessionalServiceServiceTest` | 10 | sinh slug không trùng |
| | `ServiceMapperTest` | 3 | chạy mapper MapStruct thật: chọn ảnh primary, không có primary → ảnh đầu tiên, không có ảnh → `null` |
